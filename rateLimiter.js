...
const getRateLimiter = (apiService, limit) => {
    ...
    const resolvePromiseExternally = (requestId,apiService) => {
        ...
        apiService(requestId).then(res => {
            const pResolve = promiseResolvesArr.find(k => {k.requestId = requestId})
            pResolve.resolve(res)
            numberOfAPIsInprogress--;
            console.log('----->',res,numberOfAPIsInprogress,promiseResolvesArr)
            if(promiseResolvesArr.length) {
                const firstElement = promiseResolvesArr.shift();
                resolvePromiseExternally(firstElement.requestId,apiService);
            }
        }) 
    }
    const send = requestId => {
        ...
        if(numberOfAPIsInprogress >= limit) {
            // console.log('-------->',numberOfAPIsInprogress,requestId)
        ...
        return new Promise(resolve => {
            ...
            apiService(requestId).then(res => {
                resolve(res);
                // console.log('==========',res,numberOfAPIsInprogress)
                numberOfAPIsInprogress--;
                if(promiseResolvesArr.length) {
                    const firstElement = promiseResolvesArr.shift();
                    resolvePromiseExternally(firstElement.requestId,apiService);
